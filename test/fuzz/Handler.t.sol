...
contract Handler is Test {
    ...
    uint256 MAX_DEPOSITED_SIZE = type(uint256).max;
    ...
    function mintDsc(uint256 amount,uint256 adressSeed) public {
        if( usersWithCollateralDeposited == 0){
            return;
        }
        address sender = usersWithCollateralDeposited[adressSeed % 
        ...
        amount = bound(amount,1,MAX_DEPOSITED_SIZE);
        if( amount == 0){
            return;
        }
